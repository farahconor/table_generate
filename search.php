...
$main_index = main_index_replace;
$RA_index = RA_index_replace;
$DEC_index = DEC_index_replace;
...
class conesearch
{
  ...
  function header($errorMsg, $file_name){
  ...
  function fields($catalog, $unitExceptions, $main_index, $RA_index, $DEC_index){
    ...
    for ($i=0; $i < count($catalog[0]); $i++) {
      ...
      if ($i == $main_index){
        echo '<FIELD ID="', $catalog[0][$i],'" name="', $catalog[2][$i], '" datatype="', $this->dataTyper($catalog[3][$i]), '" ucd="ID_MAIN" unit="', $unit, '"/>', "\n";
      } elseif ($i == $RA_index){
        echo '<FIELD ID="', $catalog[0][$i],'" name="', $catalog[2][$i], '" datatype="', $this->dataTyper($catalog[3][$i]), '" ucd="POS_EQ_RA_MAIN" unit="', $unit, '"/>', "\n";
      } elseif ($i == $DEC_index){
        echo '<FIELD ID="', $catalog[0][$i],'" name="', $catalog[2][$i], '" datatype="', $this->dataTyper($catalog[3][$i]), '" ucd="POS_EQ_DEC_MAIN" unit="', $unit, '"/>', "\n";
      } else {
        echo '<FIELD ID="', $catalog[0][$i],'" name="', $catalog[2][$i], '" datatype="', $this->dataTyper($catalog[3][$i]), '" unit="', $unit, '"/>', "\n";
      }
    }
    echo '<DATA>',"\n",
    '<TABLEDATA>',"\n";
  ...
  function writeData($catalog,$RA_index,$DEC_index){
...
if ($errorFlags==""){ // if there are no errors, proceedes normally, otherwise displays the error
  ...
  $xml->fields($catalog, $unitExceptions, $main_index, $RA_index, $DEC_index); // step 2
  
  $xml->writeData($catalog,$RA_index,$DEC_index); // step 3
} else{
  $errorMsg = '<INFO ID="Error" name="Error" value="' . $errorFlags . '"/>' . "\n";

  $xml->header($errorMsg=$errorMsg,$file_name); // step 1
